...
public class ControllerProtocolManager {
	...
	public synchronized void sendMessage(int destID, int messageID,
			String message, LinkedList<String> arguments) {
	...
	private synchronized void writeGameState(String message) {
		...
		try {
			...
			bufferedWriter.write("\n" + controller.getOtherPlayer().getName()
					+ " Territories\n\n");
			it = controller.getOtherPlayer().getTerritories().values()
					.iterator();
			while (it.hasNext()) {
				Territory t = it.next();
				bufferedWriter.write(t.getId() + " " + t.getName() + " - "
	...
	public void processSuccess(ConnectedPlayer cp, int messageID,
		...
		// DRAW
		if (getMesID() >= 500
				&& Math.abs(controller.getGameState().getCurrentPlayer()
						.getTerritories().size()
						- controller.getGameState().getOtherPlayer()
								.getTerritories().size()) > 10 && stillPlaying) {
			stillPlaying = false;
			LinkedList<String> tempList = new LinkedList<String>();
			String maxPlayer = "";
			if (controller.getGameState().getCurrentPlayer().getTerritories()
					.size() > controller.getGameState().getOtherPlayer()
					.getTerritories().size()) {
				maxPlayer = controller.getGameState().getCurrentPlayer()
						.getName();
			} else {
				maxPlayer = controller.getGameState().getOtherPlayer()
						.getName();
			}
			tempList.add(maxPlayer);

			controller.gameResult = tempList;
			sendMessage(controller.getGameState().getOtherPlayerID(),
					getMesID(), getCommand(11), tempList);
		} else {
			...
			case "initial_own_territories": {
				initialCount++;
				if (initialCount == 1) {
					Player player2 = controller.getGameState().getPlayers()
							.get(1);

					LinkedList<String> pt = new LinkedList<String>();
					Iterator<Territory> it = player2.getTerritories().values()
							.iterator();
					while (it.hasNext()) {
						Territory t = it.next();
						pt.add(t.getId() + "");
			...
			case "troops_placed": {
				if (controller.getPhase() == GameState.SETUP) {
					...
					if (troopPlacedCount[0] == 1 && troopPlacedCount[1] == 0) {
						sendMessage(
								controller.getGameState().getPlayers().get(1)
										.getId(), getMesID(), getCommand(6),
								controller.placed1);
					} else if (troopPlacedCount[0] == 1
							&& troopPlacedCount[1] == 1) {
						controller.starterAllocation(1);
					} else if (troopPlacedCount[0] == 2
							&& troopPlacedCount[1] == 1) {
						sendMessage(
								controller.getGameState().getPlayers().get(1)
										.getId(), getMesID(), getCommand(6),
					...
					} else if (troopPlacedCount[0] == 3
							&& troopPlacedCount[1] == 3) {
			...
			case "attack": {
				if (arguments.isEmpty()) {
					...
					attackResultCount = 0;
					controller
							.resolveAttack(arguments.get(0), arguments.get(1));
				}
				break;
			}
